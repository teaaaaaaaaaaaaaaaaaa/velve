...
import { View, Text, RefreshControl, FlatList, ScrollView, TouchableOpacity } from 'react-native';
...
export default function WishlistScreen() {
  ...
  const fetchWishlist = async () => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(unknownError, t('wishlist.loadError')));
    }
  };
                ...
                {sizes.map((size) => (
                  ...
                  >
                    <Text className="font-sans text-sm text-ink-dark">
                      {size === ALL_FILTER ? t('wishlist.allFilter') : size}
                    </Text>
                  </TouchableOpacity>
                ))}
                {(
